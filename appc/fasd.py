...
async def waitin(message: Message, state: FSMContext):
    if message.text:
        prod_session.add(Product(type="text", content=message.text, caption="no caption"))
        prod_session.commit()
    if message.photo:
        prod_session.add(Product(type="photo", content=message.photo[-1].file_id, caption=message.caption or "no caption"))
        prod_session.commit()
    if message.video:
        prod_session.add(Product(type="video", content=message.video.file_id, caption=message.caption or "no caption"))
        prod_session.commit()
...
async def waitmessage(message: Message, state: FSMContext, bot: Bot):
    ...
    for user in users:
        try:
            if message.photo:
                await bot.send_photo(chat_id=user.userid, photo=message.photo[-1].file_id, caption=message.caption or "")
            elif message.video:
                await bot.send_video(chat_id=user.userid, video=message.video.file_id, caption=message.caption or "")
            else:
...
async def waitin(message: Message, state: FSMContext):
    if message.text:
        prod_cursor.execute(
            "INSERT INTO products (type, content, caption) VALUES (?, ?, ?)",
...
async def cmd_start(message: Message):
    if message.from_user.id in admins_id:
        ...
        await message.update_data(user_id=message.from_user.id)
